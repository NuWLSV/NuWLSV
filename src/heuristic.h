...
int ISDist::pick_var()
{
    ...
    bool approve_flip_by_voting = true;
...
int ISDist::pick_var(Decimation* deci)
{
    int i, v;
    int best_var;
    int sel_c;
    ...
    update_clause_weights();
    #if VOTING_IN_SEARCH
    bool approve_flip_by_voting = false;
    while(!approve_flip_by_voting)
    {
    #endif
        if (hardunsat_stack_fill_pointer > 0)
        {
        ...
        p = clause_lit[sel_c];
        // #if VOTING_IN_SEARCH
        // best_var = best_var_by_score_voting(p, deci, sel_c, 4, 1);
        // #else
        for (p++; (v = p->var_num) != 0; p++)
        {
            if (score[v] > score[best_var])
        ...
        // #endif
    #if VOTING_IN_SEARCH
        approve_flip_by_voting = flip_or_not_by_voting(best_var, deci);
    }
    #endif
        ...
        {   
            ...
            if (hard_unsat_nb == 0)
            {
                ...
                if (soft_unsat_weight < opt_unsat_weight)
                {
                    ...
                    deci.push_global_opt_soln(cur_soln, soft_unsat_weight);
...
void ISDist::local_search_with_decimation(char *inputfile, int sample_num, int threshold, int max_num_global_opt_solns)
{
    if (1 == problem_weighted)
    {
        if (total_soft_length / num_sclauses > 100)
    ...
    Decimation deci(var_lit, var_lit_count, clause_lit, org_clause_weight, top_clause_weight, sample_num, threshold, max_num_global_opt_solns);
    deci.make_space(num_clauses, num_vars);

    opt_unsat_weight = __LONG_LONG_MAX__;
    for (tries = 1; tries < max_tries; ++tries)
    {
        deci.init(local_opt_soln, best_soln, unit_clause, unit_clause_count, clause_lit_count); // some problems here
        deci.unit_prosess();
        init(deci.fix);
        ...
        for (step = 1; step < max_flips; ++step)
        {
            if (hard_unsat_nb == 0)
            {
                ...
                if (soft_unsat_weight < opt_unsat_weight)
                {
                    opt_time = get_runtime();
                    opt_unsat_weight = soft_unsat_weight;
                    #if VOTING_IN_SEARCH || USING_VOTING_IN_UNIT_PROCESS
                    deci.push_global_opt_soln(cur_soln, soft_unsat_weight);
                    #endif
...
bool ISDist::flip_or_not_by_voting(int v, Decimation* deci)
{
    if(deci -> num_global_opt_solns != 0){
        if(deci -> voting_by_local_opt(v) == cur_soln[v]){
            return false;
        }
    }
    return true;
...
int  ISDist::best_var_by_score_voting(lit* clause_lit, Decimation* deci, int sel_c, int alpha, int beta){
    int i, v, sense, j = 0;
    int cnt[2] = {0, 0};
    int num_global_opt_solns_tmp = deci -> num_global_opt_solns;

    int pick_var = -1;
    int clause_len = clause_lit_count[sel_c];
    double* new_score = new double[clause_len];
    double sum_scores = 0;

    /* calculate the new score according to the two scoring scheme*/
    for(lit* p = clause_lit;(v = p -> var_num) != 0; p++)
    {
        cnt[0] = 0, cnt[1] = 0;
        sense = (cur_soln[v] == p -> sense)? 0: 1;
        new_score[j] = pow(2, deci -> global_var_val_cnt[v][sense] * alpha + score[v] * beta);
        sum_scores += new_score[j];
        j += 1;
    }

    /* each var is assigned a possibility according to its new score*/
    double rand_num = rand() / double(RAND_MAX - 1); // a number in the range [0, 1)
    for(j = 0;j < clause_len;j++)
    {
        double p = new_score[j] / sum_scores;
        if(rand_num < p){
            pick_var = clause_lit[j].var_num;
            break;
        }
        rand_num -= p;
    }
    if(pick_var == -1){
        pick_var = clause_lit[clause_len - 1].var_num;
    }
    delete new_score;
    return pick_var; 
} 
