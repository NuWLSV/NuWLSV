...
class ISDist
{
  private:
	...
	int best_var_by_score_voting(lit* clause_lit, Decimation* deci, int sel_c, int alpha, int beta);
	...
	int pick_var();
	int pick_var(Decimation* deci);
	bool flip_or_not_by_voting(int v, Decimation* deci);
  public:
	double opt_time;
	long long opt_unsat_weight;
	bool global_sol_feasible;
	
	ISDist();
	void settings();
	void build_instance(char *filename);
	void local_search_with_decimation(char *inputfile);
	void local_search_with_decimation(char *, int, int, int);
	void simple_print();
